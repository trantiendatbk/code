...
class AutoKey_Cipher extends Component {
    constructor(props) {
        super(props)
    ...
    Cryp = () => {
        ...
        console.log('vao day')
        var i = []
        i = Object.values(this.state.input)
        // console.log('vao day ii' + i[0].toString())
        this.setState({ show: true })
        console.log('show la :' + this.state.show)
        i.forEach((element, index1) => {


            let m = element.toString()
            let m1 = m.charCodeAt(0) - 97

            console.log('vao day2' + element.toString() + "index 1------" + index1 + "m la-----" + m)
            console.log("index la" + m1)

            let index = m.charCodeAt(0) - 97;
            console.log('index trn la::' + index)

            if (index1 == 0) {
                var convert = (index + parseInt(this.state.key)) % 26 + 97
            } else {
                console.log("gia tri i la" + i[index1 - 1])
                // let indexOfelement= i.indexOf(element)
                let preE = i[index1 - 1].charCodeAt(0) - 97
                console.log("gia tri preE" + preE)
                var convert = (m1 + preE) % 26 + 97
            }
            console.log('gia tri convert' + convert)
